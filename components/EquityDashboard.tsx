  ...
  },
  {
    ...
    icon: (
      <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
        ...
        <line x1="3" y1="10" x2="21" y2="10" /><line x1="7" y1="15" x2="11" y2="15" />
      </svg>
    ),
  },
  {
...
export default function EquityDashboard() {
  ...
  useEffect(() => {
    async function load() {
      const [props, morts, pays] = await Promise.all([
        fetchTable<Property>("properties", "created_at.asc"),
        fetchTable<Mortgage>("mortgages"),
        fetchTable<Payment>("payments", "month.desc"),
      ]);
      setProperties(props);
      setMortgages(morts);
      setPayments(pays);
  ...
  const filteredPayments = activeFilter
    ? payments.filter((p) => p.property_id === activeFilter)
    : payments;
      ...
      <main style={{ marginLeft: 78, padding: "40px 48px 160px", maxWidth: 1140 }}>
        ...
        <section id="dashboard" style={{ scrollMarginTop: 28 }}>
          <div style={{ background: "#1f3d2e", borderRadius: 14, padding: "38px 42px" }}>
            {loading ? (
              <div style={{ color: "#9db8a6", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 15 }}>Načítám data…</div>
            ) : (
              <div className="flex justify-between items-start gap-10">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-[14px] mt-[22px]">
                    ...
                    <span style={{ fontSize: 15, color: "#cfe0d4", fontWeight: 500 }}>
                      Hodnota portfolia {fmtMil(totalValue)} mil Kč
                    </span>
                  </div>
                  {totalDebt > 0 && (
        ...
        <section id="nemovitosti" style={{ marginTop: 38, scrollMarginTop: 28 }}>
          ...
          {loading ? (
            <div style={{ color: "#7c8378" }}>Načítám…</div>
          ) : (
              ...
              {activeProperties.map((p) => {
                ...
                return (
                  <div key={p.id} className="cursor-pointer"
                    style={{ background: "#f5f1e6", borderRadius: 10, padding: "15px 18px", border: p.status === "planned" ? "1px dashed #c9c0aa" : "1px solid transparent" }}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <span style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600, fontSize: 15, color: "#1c2b22" }}>
                          {fmtMil(p.estimated_value)} mil
                        </span>
                        <span className={`inline-flex items-center rounded-[20px] ${cls}`}
                          style={{ fontWeight: 600, fontSize: 11, letterSpacing: "0.04em", textTransform: "uppercase", padding: "5px 11px" }}>
                          {label}
                        </span>
                      </div>
                    </div>
                          ...
                          return (
                            <div>
                              <div className="flex justify-between" style={{ fontSize: 11, color: "#9a9483", marginBottom: 4 }}>
                                <span>LTV</span>
                                <span style={{ color, fontWeight: 600 }}>{ltv} %</span>
                              </div>
                              ...
                              <div style={{ fontSize: 11, color: "#9a9483", marginTop: 3 }}>
                                Dluh {fmtMil(mortgage.outstanding_balance)} mil · Hodnota {fmtMil(p.estimated_value)} mil
                              </div>
                            </div>
                          );
                        })()}
        ...
        <section id="platby" style={{ marginTop: 38, scrollMarginTop: 28 }}>
          ...
          {loading ? (
            ...
            <div style={{ background: "#f5f1e6", borderRadius: 10, padding: "24px 20px", color: "#9a9483", fontSize: 14 }}>
              Žádné platby zatím. Přidej je přes Supabase nebo rozhraní.
            </div>
          ) : (
            <div style={{ background: "#f5f1e6", borderRadius: 10, padding: "6px 20px" }}>
              ...
              {filteredPayments.map((p) => (
                <div key={p.id} className="flex" style={{ fontVariantNumeric: "tabular-nums", fontSize: 14, padding: "13px 0", borderTop: "1px solid #e3ddcb", color: "#1c2b22" }}>
