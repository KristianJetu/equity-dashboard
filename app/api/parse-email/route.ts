...
// Server-side endpoint chráněný tajným klíčem — zapisuje přes service role (obchází RLS),
// protože anon nemá SELECT na payments (INSERT ... RETURNING by jinak spadl na RLS).
...
- sender_account: číslo účtu odesílatele z "z uc. ..." (přesně jak je v emailu)
- note: obsah "AV:" pole (variabilní symbol / zpráva), pokud existuje
- payment_type: "rent" (běžný nájem), "deposit" (kauce), "partial" (nižší nájem), "other"
...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = await req.json() as { emailText?: string };
    const emailText = body.emailText?.trim() ?? "";

    if (!emailText) {
      return NextResponse.json({ ok: false, error: "no email text" }, { status: 400 });
    }
