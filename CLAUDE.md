...
- **UI**: `ProjectionPreviewModal` — velké modální okno (ne malý dialog), otevírané odkazem "⚙ Upravit předpoklady" u vysvětlivky Optimistické projekce (zobrazí se jen když je Optimistická vybraná a Finanční profil zapnutý). Dva sloupce jako samostatná ověřovací kalkulačka (viz níže): vlevo všechny vstupy jako posuvníky s živou hodnotou (včetně "Osobní a příjmové vstupy" — rok narození/příjmy/náklady/LTV, které se ukládají do stejných sloupců `profiles` jako v Nastavení → Finanční profil, jen zpřístupněné i tady), vpravo živě přepočítávaný náhled (souhrnné dlaždice, graf hodnota/dluh/majetek se značkami akvizic, tabulka rok po roce s DSTI/DTI/zůstatkem/akvizicemi) na skutečných datech portfolia — barvy a fonty appky, ne kalkulačky.
